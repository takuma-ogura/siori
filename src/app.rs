...
pub struct App {
    ...
    pub visual_list: Vec<VisualEntry>,
...
impl App {
    ...
    fn push(&mut self) -> Result<()> {
        ...
        if output.status.success() {
            ...
            if err.contains("No configured push destination") || err.contains("does not appear to be a git repository") {
                self.input_mode = InputMode::RemoteUrl;
                self.remote_url.clear();
                self.message = Some(("No remote configured. Enter repository URL:".to_string(), true));
                return Ok(());
            }
